...
public class ReadyPlayerAvatar : MonoBehaviour
{
    ...
    // Avatar filename sitting directly inside the StreamingAssets folder root
    public String localFilename = "67e21d1a79ac9bcf81a46385.glb";
    ...
    private async void InitializeAvatar()
    {
        ...
        if (avatar_url.Length == 0)
        {
            // 1. Build path to StreamingAssets root
            string path = System.IO.Path.Combine(Application.streamingAssetsPath, localFilename);
            
            // 2. WebGL/glTFast requires forward slashes instead of Windows backslashes
            path = path.Replace("\\", "/");

            // 3. Local editor needs the protocol scheme prefix or glTFast treats it as an invalid URL
#if UNITY_EDITOR
            if (!path.Contains("://"))
            {
                path = "file:///" + path;
...
#endif
            Debug.Log("Loading Local Model from: " + path);
            await gltfImport.Load(path);
        }
        else
        {
